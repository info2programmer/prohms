...
<head>
    <meta charset="UTF-8">
    <title>Payslip - <?php echo $rows->emp_code;?>-<?php echo date_format(date_create($rows->from_date), "F"); ?>-<?php echo date_format(date_create($rows->from_date), "Y"); ?></title>
    <style>
        body {
            padding: 10px;
            margin: 0
        ...
        body > table {background: url(<?php echo base_url().'uploads/'.$rows_company->com_logo?>) 9px 9px no-repeat; background-size: auto 50px; width: 8in;}
...
<table align="center">
    ...
    <tr>
        ...
        <td>:</td>
        <td><?php echo date_format(date_create($rows->joining_date), "d-m-Y");?></td>
        ...
        <td width="50%" colspan="3" class="no-pad bordered no-left-border">
            <table>
                <tr>
					...
					?>
                    <td width="75%" class="subheader extra-pad">Late Details ( <?php echo $late_count; ?> )</td>
        ...
        <td width="50%" colspan="3" class="no-pad bordered no-left-border no-right-border">
            <table>
                <tr>
					...
					?>
                    <td width="50%" class="subheader extra-pad">Absent Details ( <?php echo $absent_count; ?> )</td>
...
?>
    ...
    <tr class="bottom-border">
        <td colspan="2" class="extra-pad">
            <strong>Leave Balance :</strong></td>
        <td colspan="4" class="extra-pad">
        <?php
		$leaves = $this->db->query("select a.*,b.* from td_leave_allocation as a inner join td_leave_allocation_details as b on b.leave_allocation_id=a.leave_allocation_id where a.emp_id='$rows->emp_id'")->result();
		if($leaves) { foreach($leaves as $leave) {
			$leaveDetail = $this->db->query("select * from td_leave where leave_id='$leave->leave_id'")->row();
		?>
            <u><strong><?php echo $leaveDetail->leave_name; ?></strong></u> :  <?php echo $leave->leave_balance; ?><br>
        <?php } } ?>
        </td>    
    </tr>
    <tr>
