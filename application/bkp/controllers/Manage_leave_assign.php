...
class Manage_leave_assign extends CI_Controller {
...

	{
		...
		$data['maincontent']=$this->load->view('maincontents/manage-leave-assign-list-view',$data,true);
...

	{
		...
		$data['rows'] = $this->db->query("select * from td_leave_assign where emp_id='$id' and published=1 order by leave_date desc")->result();
	...
	{	
...

		{
			...
			{				
				...
				{	
					...
					$new_leave_balance_assign = $old_leave_balance-1;
					$succ = $this->db->query("UPDATE `td_leave_assign` SET `leave_left`='$new_leave_balance_assign' WHERE leave_type='$leave_allocation_id' and leave_assign_id='$insert_id'");
		...
		$data['maincontent']=$this->load->view('maincontents/add-edit-leave-assign-view',$data,true);
