...
class Manage_salary extends CI_Controller {
	...
	public function employee_salary()
	{
		...
		$data['left_sidebar'] = $this->load->view('elements/left-sidebar','',true);
		$data['footer'] = $this->load->view('elements/footer','',true);
		$data['maincontent']=$this->load->view('maincontents/employee-salary-view',$data,true);
		$this->load->view('layout-after-login',$data);
	}
	...
	function salary_report()
	{
		...
		$data['left_sidebar'] = $this->load->view('elements/left-sidebar','',true);
		$data['footer'] = $this->load->view('elements/footer','',true);
		$data['maincontent']=$this->load->view('maincontents/salary-report-view',$data,true);
		$this->load->view('layout-after-login',$data);
	}
		...
		if($this->input->post('mode')=='tab')
		{
			...
			{				
				...
				if($data)
				{
					...
					redirect('manage_salary/salary_confirm/'.$insert_id);					
				}
				else
				{
	...
	function salary_confirm($id)
	{
		...
		if($this->input->post('mode')=='tab')
		{
			...
			for($i=0;$i<count($salary_head_values);$i++)
			{
					...
					//echo "UPDATE `td_salary_details` SET `salary_head_value`='$salary_head_value' WHERE `salary_id`='$id' and `salary_head_id`='$salary_head_id'";
					//echo '<br>';
			}
		...
		$data['left_sidebar'] = $this->load->view('elements/left-sidebar','',true);
		$data['footer'] = $this->load->view('elements/footer','',true);
		$data['maincontent']=$this->load->view('maincontents/add-edit-salary2-view',$data,true);
		$this->load->view('layout-after-login',$data);
	}
	######################################################################################
        ...
        if (isset($_POST) && isset($_POST['emp_id'])) 
		{
			...
			if($emp_type=='S') {
				$new_row = '<div class="form-group">
                				...
                				<div class="col-lg-9">
                  					<input type="number" name="basic_pay" value="'.$basic_pay.'" class="form-control" placeholder="Enter Basic Pay">
