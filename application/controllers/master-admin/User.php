...
class User extends CI_Controller {
	...
	{	
		...
		$data['maincontent']=$this->load->view('master-admin/maincontents/home',$data,true);
	...
	public function login()
	{
			...
			else
			{
				...
				if($record)
				{
							$sessiondata2 = array(
												...
												'is_admin_logged_in2' => true
												);												

							$this->session->set_userdata($sessiondata2);
							//echo '<pre>';print_r($sessiondata2);
							...
							if($this->session->userdata('is_admin_logged_in2') == 1)
							{
								...
								header("Refresh:2;url=".base_url()."master-admin/user");
		...
		$this->load->view('master-admin/layout-before-login');	
	}
	...
	function change_password()
	{
		...
		{	
			if($this->password_validate() == FALSE)
				...
				else
				{
							...
							else if($this->session->userdata('user_id2'))
							{
							...
							{	
								...
								redirect('master-admin/user/logout');
							}
							else
							{	
		...
		$data['left_sidebar'] = $this->load->view('master-admin/elements/left-sidebar','',true);
		$data['footer'] = $this->load->view('master-admin/elements/footer','',true);
		$data['maincontent']=$this->load->view('master-admin/maincontents/add-edit-password-view',$data,true);
		$this->load->view('master-admin/layout-after-login',$data);
	}
	...
	function existing_password($str)
	{
		...
		{
		$user_id1 = $this->session->userdata('user_id2');	
		}
