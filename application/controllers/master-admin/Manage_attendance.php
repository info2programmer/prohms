...
class Manage_attendance extends CI_Controller {
	...
	function index()
	{
			...
			$order_by[0] = array('field'=>'attendance_date','type'=>'ASC');
		...
		$data['maincontent']=$this->load->view('master-admin/maincontents/manage-attendance-list-view',$data,true);
	...
	function attendance_report()
	{
		if($this->input->post('mode')=='tab') {
			...
			$table['name'] = 'td_attendance';
			$order_by[0] = array('field'=>'td_attendance.attendance_date','type'=>'ASC');
			$conditions = array('td_attendance.attendance_date>='=>$from_date,'td_attendance.attendance_date<='=>$to_date);
		...
		$data['left_sidebar'] = $this->load->view('master-admin/elements/left-sidebar','',true);
		$data['footer'] = $this->load->view('master-admin/elements/footer','',true);
		$data['maincontent']=$this->load->view('master-admin/maincontents/attendance-report-view',$data,true);
		$this->load->view('master-admin/layout-after-login',$data);	
	}
	
	function attendance_sheet($id)
	{
		$table['name'] = 'td_attendance';
		$conditions = array('td_attendance.attendance_id>='=>$id);
		$data['row1'] = $this->Common_model->find_data($table,'row','',$conditions);
		
		$data['rows'] = $this->db->query("select * from td_employee_personal_details where published=1")->result();
		
		$data['head'] = $this->load->view('master-admin/elements/head','',true);
		$data['header'] = $this->load->view('master-admin/elements/header','',true);
		$data['left_sidebar'] = $this->load->view('master-admin/elements/left-sidebar','',true);
		$data['footer'] = $this->load->view('master-admin/elements/footer','',true);
		$data['maincontent']=$this->load->view('master-admin/maincontents/daily-attendance-view',$data,true);
		$this->load->view('master-admin/layout-after-login',$data);
	}
	...
	{	
		...
		if($this->input->post('mode')=='tab')
		{
			...
			{				
				...
				{	
					...
					redirect('master-admin/manage_attendance');					
				}
				else
				{
					redirect('master-admin/manage_attendance');	
		...
		$data['left_sidebar'] = $this->load->view('master-admin/elements/left-sidebar','',true);
		$data['footer'] = $this->load->view('master-admin/elements/footer','',true);
		$data['maincontent']=$this->load->view('master-admin/maincontents/add-edit-attendance-view',$data,true);
		$this->load->view('master-admin/layout-after-login',$data);
	}	
